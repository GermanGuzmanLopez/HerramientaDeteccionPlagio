...
Version definitva candidata a optimización futura.
"""
...
if not archivo_tiene_contenido(ruta_vectores):
    archivos, vectores, vectorizador = analizar_documentos_carpeta(
        "./data/textos_origen")
else:
...
for archivo in os.listdir(carpeta_test_data):
    if archivo.endswith(".txt"):
        with open(os.path.join(carpeta_test_data, archivo), 'r', encoding='utf-8') as file:
            ...
            similitudes = comparar_nuevo_texto(
                nuevo_texto, archivos, vectores, vectorizador)
            resultado = {
                'Archivo': archivo,
...
# Evaluar umbrales
metricas_df, umbral_optimo = evaluar_umbral(
    resultados_similitudes, plagios_reales)
...
tabla_resultados_top20 = tabla_resultados_df.sort_values(
    by='Porcentaje de Plagio', ascending=False)
